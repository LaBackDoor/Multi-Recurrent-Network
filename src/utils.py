...
def create_mrn_from_structure(
    ...
    if len(nn_structure) < 3:
        raise ValueError(f"nn_structure must have at least 3 elements, got {len(nn_structure)}")

    return MRN(
        nn_structure=nn_structure,
        memory_structure=mm_structure,
        device=device
    )
...
def print_mrn_info(model: "MRN"): # FIX 4: Use string "MRN" for type hint
    ...
    for i in range(model.num_layers):
        layer_type = "Input" if i == 0 else ("Output" if i == model.num_layers - 1 else f"Hidden {i}")
        mem_info = f"{model.cell.memory_structure[i]} memories" if model.cell.memory_structure[i] > 0 else "no memory"
    ...
    if len(model.cell.memory_banks) == 0:
        ...
        for layer_idx, bank in sorted(model.cell.memory_banks.items(), key=lambda x: int(x[0])):
            layer_num = int(layer_idx)
            layer_type = "Input" if layer_num == 0 else ("Output" if layer_num == model.num_layers - 1 else f"Hidden {layer_num}")
            print(f"  Layer {layer_idx} ({layer_type}): {bank.num_items} items × {bank.layer_size} dims → {bank.output_size} output")
